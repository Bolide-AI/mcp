...
const UsePerplexitySchema = z.object({
  ...
  search_mode: z
    .enum(['web', 'academic'])
    .describe(
      `Controls the search mode used for the request. When set to 'academic', results will prioritize scholarly sources like peer-reviewed papers and academic journals. (default: web)`,
    ),
});
...
async function usePerplexity(params: UsePerplexityParams): Promise<ToolResponse> {
  ...
  log(
    'info',
    `Performing (${search_mode}) search with Perplexity via web API on query: "${query}"`,
  );
  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (response.status === 401) {
        ...
        throw new ValidationError(
          `Failed to perform search via web API: ${response.status} ${response.statusText}`,
        );
      }
    }
    ...
    log(
      'info',
      `Search completed successfully via web API. Response length: ${result.length} characters`,
    );
...
async function useOpenAIDeepResearch(params: UseOpenAIDeepResearchParams): Promise<ToolResponse> {
  ...
  log(
    'info',
    `Performing deep research with OpenAI o4-mini-deep-research via web API on query: "${query}"`,
  );
  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (response.status === 401) {
        ...
        throw new ValidationError(
          `Failed to perform deep research via web API: ${response.status} ${response.statusText}`,
        );
      }
    }
    ...
    log(
      'info',
      `Deep research completed successfully via web API. Response length: ${result.length} characters`,
    );
